...
contract('BFactory', async (accounts) => {
    ...
    describe('Factory', () => {
        ...
        //     await truffleAssert.reverts(factory.collect(nonAdmin, { from: nonAdmin }), '!governance && !collectedFund');
        // });
        ...
        // it('nonadmin cant set vlab address', async () => {
        //     await truffleAssert.reverts(factory.setVlab(nonAdmin, { from: nonAdmin }), '!vlab');
        // });

        it('admin changes governance address', async () => {
            await factory.setGovernance(user2);
            const governance = await factory.governance();
            assert.equal(governance, user2);
        });
    });
});
