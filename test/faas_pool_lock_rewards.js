...
contract('faas_pool_lock_rewards', async (accounts) => {
    ...
    async function printRewardPoolInfo(pid) {
        ...
        console.log('rewardPool[%d] accRewardPerShare: ', pid, String(rewardPool.lastRewardBlock));
    ...
    async function printStakeInfo(name, account, pid) {
        ...
        //         mapping(uint8 => uint) lockRewardPending;
        //         mapping(uint8 => uint) lockRewardReleased;
        //         uint lastStakeTime;
        // console.log('%s amount:            %s', name, fromWei(user.amount));
        // if (pool.getRewardDebt) console.log('%s rewardDebt[%d]:    %s', name, pid, String(await pool.getRewardDebt(pid, account)));
        // if (pool.getLockRewardPending) console.log('%s lockRewardPending[%d]:    %s', name, pid, String(await pool.getLockRewardPending(pid, account)));
        // if (pool.getLockRewardReleased) console.log('%s lockRewardReleased[%d]:    %s', name, pid, String(await pool.getLockRewardReleased(pid, account)));
        console.log('%s pendingReward[%d]: %s', name, pid, String(await pool.pendingReward(pid, account)));
    }

    describe('RewardPool with lock', () => {
        it('_endRewardBlock < _startVestingBlock', async () => {
            ...
            const _startBlock = currBlk + 10;
            const _endRewardBlock = currBlk + 20;
            const _rewardPerBlock = toWei('1');
            const _lockRewardPercent = 90;
            const _startVestingBlock = currBlk + 30;
            const _endVestingBlock = currBlk + 50; // 10% every 2 blocks
            ...
            await pool.joinPoolFor(carol, toWei('20'), [MAX, MAX], {from: bob});
            ...
            await expectRevert(
                pool.exitPool(toWei('10'), [0, 0], {from: carol}),
                'frozen'
            );
            await expectRevert(
                pool.setUnstakingFrozenTime(0, {from: bob}),
                '!cntler'
            );
            await pool.setController(bob);
            await pool.setUnstakingFrozenTime(0, {from: bob});
            await pool.withdraw(toWei('10'), {from: carol});

            if (verbose) {
                console.log('\n=== AFTER exitPool: Block: %s ===', await time.latestBlock());
                await printBalances();
                await printRewardPoolInfo(0);
                await printStakeInfo('bob', bob, 0);
            }
            ...
            await pool.getAllRewards({from: bob});

            if (verbose) {
                console.log('\n=== AFTER getAllRewards: Block: %s ===', await time.latestBlock());
                await printBalances();
                await printRewardPoolInfo(0);
                await printStakeInfo('bob', bob, 0);
            }
            ...
            if (verbose) {
                console.log('\n=== AFTER getAllRewards (after endVestingBlock): Block: %s ===', await time.latestBlock());
                await printBalances();
                await printRewardPoolInfo(0);
                await printStakeInfo('bob', bob, 0);
            }
