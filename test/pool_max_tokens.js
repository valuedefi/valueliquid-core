...
contract('pool_max_tokens', async (accounts) => {
    ...
    describe('Binding Tokens', () => {
        ...
        it('Fails binding more than 8 tokens', async () => {
            await truffleAssert.reverts(pool.bind(ZZZ, toWei('50'), toWei('2')), '>maxTokens');
        });
        ...
        it('Fails gulp on unbound token', async () => {
            await truffleAssert.reverts(pool.gulp(ZZZ), '!bound');
        });
