...
contract('pool', async (accounts) => {
    ...
    describe('Finalizing pool', () => {
        ...
        it('Only controller can setPublicSwap', async () => {
            ...
            const publicSwap = pool.publicSwap();
            assert(publicSwap);
            await truffleAssert.reverts(pool.setPublicSwap(true, { from: user1 }), '!controller');
        });
        ...
        it('Cant setPublicSwap, setSwapFee when finalized', async () => {
            await truffleAssert.reverts(pool.setPublicSwap(false), 'finalized');
            await truffleAssert.reverts(pool.setSwapFee(toWei('0.01')), 'finalized');
        });
    ...
    describe('User interactions', () => {
        ...
        */
        ...
        it('Fails admin unbinding token after finalized and others joined', async () => {
            await truffleAssert.reverts(pool.unbind(DAI), 'finalized');
        });
    ...
    describe('BToken interactions', () => {
        it('Token descriptors', async () => {
            ...
            assert.equal(name, 'Value Liquidity Provider');

            const symbol = await pool.symbol();
            assert.equal(symbol, 'VLP');
