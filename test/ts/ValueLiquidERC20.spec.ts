...
import {ValueLiquidErc20WrapperFactory} from "../../typechain";


const TOTAL_SUPPLY = toWei(10000)
const TEST_AMOUNT = toWei(10)

describe('ValueLiquidErc20', () => {
	...
	beforeEach(async () => {
		...
		wallet = signers[0];
		other = signers[1];
		token = await new ValueLiquidErc20WrapperFactory(wallet).deploy('Value Liquidity Provider V2','VLP-V2', TOTAL_SUPPLY)
	})

	it('name, symbol, decimals, totalSupply, balanceOf, DOMAIN_SEPARATOR, PERMIT_TYPEHASH', async () => {
		const name = await token.name()
		expect(name).to.eq('Value Liquidity Provider V2')
		expect(await token.symbol()).to.eq('VLP-V2')
